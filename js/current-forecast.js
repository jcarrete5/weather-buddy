...
$(document).ready(function() {
	...
	if (!queryStr.has("loc-query") || queryStr.has("loc-query") && queryStr.get("loc-query") === "") {
		...
		navigator.geolocation.getCurrentPosition(function(pos) {
			...
			$.get(apiUrl+"/geolookup/q/"+lat+","+long+".json", null, function(data, textStatus, jqXHR) {
				...
				console.log(data);
				$.get(apiUrl+"/conditions"+data.location.l+".json", null, function(data, textStatus, jqXHR) {
		...
		autocomplete(queryStr.get("loc-query"));
	}
});
